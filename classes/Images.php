<?php
/**
*This class adds different features to post edit administration panel 
*/
...
public static function getOtherImage($src, $size){
    
    preg_match("/\/uploads\/(.*)\//", $src, $mtch);
            $pathname = $mtch[1];
            preg_match("/\/uploads\/(.*)\/(.*)\/(.*)\./", $src, $match);
	    $dir = ABSPATH . '/wp-content/uploads/'.$pathname.'/';
        
$pic=$match[3].'-'.$size;
if (is_dir($dir)) {
    if ($dh = opendir($dir)) {
        while (false !== ($file = readdir($dh))) { 
        if(substr_count($file,$pic) == 1){
            $return=$file;
...
if (isset($return)) {return WP_CONTENT_URL.'/uploads/'.$pathname.'/'.$return;}
else {return $src;}
...
public static function getPostImageUrl(){
	global $post;
	  if (class_exists('Images')) {
			  ...
			  if(Images::get_post_thumbnail_url('post', 'post-image')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('post', 'post-image');
			  }
			  
			  
	 }
	 // return 'http://womenfreebies.net/tools/watermark/wm.php?lang=free&src='.$imgsrc;
	  return $imgsrc;
}

public static function getContestFirstImage(){
	global $post;
	  if (class_exists('Images')) {
		      
			  if(Images::get_post_thumbnail_url('post', 'contestfirstpage-image')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('post', 'contestfirstpage-image');
			  }
		}
	 // return 'http://womenfreebies.net/tools/watermark/wm.php?lang=free&src='.$imgsrc;
	Images::getOtherImage($imgsrc, 255);
                return $imgsrc;
}
public function get_expired($expired=FALSE){
if ($expired==TRUE) {
return 'expiredflap';
}
 return FALSE;
...
public function get_featured_image($expired=FALSE, $brand=FALSE){
...
foreach ($terms as $brands){
   ...
   if(empty($img)){
    $img='http://parentdeals.ca/free.ca/wp-content/uploads/default300-200.jpg';
	}
     if ($expired==TRUE) {

$img='http://womenfreebies.net/tools/watermark/wm.php?lang=free&src='.$img;
}
 return $img;
}

public function get_thumb_225($expired=FALSE, $brand=FALSE){
...
public function get_thumb_300($expired=FALSE){
...
public function get_thumb_470($expired=FALSE){
...
public static function getContestForstPageImageUrl(){
	global $post;
	  if (class_exists('Images')) {
...
public static function getSurveyImageUrl (){
	  global $post;
	  if (class_exists('Images')) {
		      
			  if(Images::get_post_thumbnail_url('get_survey', 'survey-image')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('get_survey', 'survey-image');
			  }
...
public static function getSurveyLogoUrl (){
	  global $post;
	  if (class_exists('Images')) {
		      
			  if(Images::get_post_thumbnail_url('get_survey', 'survey-logo')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('get_survey', 'survey-logo');
			  }
...
public static function get_facebook_image_url(){
	  if (is_single()){
	  global $post;
	  if (class_exists('Images')) {
		      if(Images::get_post_thumbnail_url('post', 'facebook-image')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('post', 'facebook-image');
			  }
			  else if(Images::get_post_thumbnail_url('post', 'post-image')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('post', 'post-image');
			  }
...
public static function get_pinterest_image_url(){
	global $post;
	  if (class_exists('Images')) {
		      if(Images::get_post_thumbnail_url('post', 'pinterest-image')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('post', 'pinterest-image');
			  }
			  else if(Images::get_post_thumbnail_url('post', 'post-image')!=""){
			  $imgsrc=Images::get_post_thumbnail_url('post', 'post-image');
			  }
...
public function register($args = array()) {
			...
			// add theme support if not already added 'Featured Image'
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			add_thickbox();
			wp_enqueue_script( "featured-image-custom", $this->theme_url( 'js/admin-page.js'), array( 'jquery', 'media-upload' ) );
       ...
       public function theme_url($relative_path) {
			$url=get_template_directory_uri();
			return $url.DS.$relative_path;
		}
